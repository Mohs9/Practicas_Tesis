...
los operadores de densidad. Así como se presenta la formulación de los
...
este y por ahí busca en lo que sigue a ver si lo vuelves a usar al inicio de una frase.}
...
ventaja que es más fácil trabajar en la descripción de sistemas cuánticos cuyo
estado no se conoce y la descripción de subsistemas de un sistema cuántico
compuesto\cpnote{Acá hay como una contradiccion. Dice que el estado no se conoce, pero en la linea 
...
Ahora, será útil introducir algunos conceptos y hechos sobre operadores de
densidad. Un sistema cuántico cuyo estado es conocido exactamente, se dice que
...
de este uso puede ser que no se asume necesariamente que un estado es puro. Y
además, el término <<estado puro>> se usa a menudo en referencia a un vector de
estado $|\psi\rangle $, para distinguirlo de un operador de densidad	$\rho$.
}; se dice que es una mezcla de los diferentes estado puros en el ensamble de
$\rho$ {\cite{nielsen_chuang_2010}}. 

Estos términos dan pie a incluir las siguientes definiciones {\cite{wilde2011classical}}: 
\begin{definition}[\textbf{Estado de máxima mezcla} \cpnote{Normalmente se le dice 
estado máximamente mezclado. O donde lo viste asi?}] El estado de máxima mezcla
...
La pureza es una medida particular del ruido de un estado cuántico. \cpnote{Esta frase está muy rara. De donde la sacas? Que es el ruido de un estado?}
...
Y \cpnote{Creo que sobra el ``Y''} dado que $p_i^2<p_i$ para todo $p_i<1$ \cpnote{para $p_i=0$ 
se cumple la igualdad...}, la
igualdad se mantiene si y solo si
la sumatoria tiene un único $p_i$ que debe ser igual a 1. Luego
$\tr(\rho^2)=\sum_{i}p_i^2=\sum_i p_i= p_i=1$ entonces
$\rho=|\psi_i\rala\psi_i|$ es puro \cpnote{Preferira quitar la ecuacion. Es confuso, 
porque es solo para este caso}. Y \cpnote{También esta raro 
comenzar la frase con Y en este caso} la pureza entonces será estrictamente
menor a uno  $\tr(\rho^2)=\sum_{i}p_i^2<\sum_i p_i=1$, si todo $p_i<1$ y
$\rho=\sum_i p_i |\psi_i\rangle \langle \psi_i|$ es mixto.
\cpnote{Porfa arregla este parrafo. Creo qe lo escribiste cansada. }
    
     
Conversamente \cpnote{Esta palabra no existe en español, segun la RAE} si $\rho
$ es puro, luego $\rho=|\psi\rala \psi|$
\[\tr(\rho^2)=\tr(|\psi\rala \psi|\psi\rala \psi|)=\tr(|\psi\rala
\psi|)=\la\psi|\psi\ra=1.\]
Y \cpnote{Trata de quitar todas las fraces que comiences con ``Y'' mas bien ver si 
lo upuedes uitar o substituir.} si $\rho $ es mixto, luego $\rho=\sum_i
...
información y computación cuántica. Y para ello se presenta el siguiente
...
tienes ahi, pero si estan ahi de manera implicita.}
	
Conversamente, \cpnote{revisa en todo el documento donde usas esa palabra} se supone que ambos conjuntos generan el mismo operador
...
$|\psi_i\ra=\sum_{k}v_{ik}|k\ra  $, donde se han agregado  vectores cero a la
...
u_{ij}|\phi_j\ra$, donde  $u=vw^\dagger$ es unitaria. 
\end{proof}
\cpnote{Veo muy complicada esta prueba. De donde la sacaste? Porque así de larga?}
...
el sistema $B$. Y \cpnote{``Y''} la traza parcial tiene la siguiente definición, presentada
...
Hay un ``box'' en el chuang donde discuten eso. Porfa incluye, en tu lenguaje y citando apropiadamente, esa discusión. }
