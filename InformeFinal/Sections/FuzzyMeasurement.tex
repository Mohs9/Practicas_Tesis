...
Sea $S$ el conjunto de pares ordenados $(a_j,b_k)$ de valores propios tales que sus respectivos operadores de proyección $|a_j b_k \rala a_j b_k|\ne|b_k a_j \rala b_k a_j |$. Entonces se introduce un nuevo operador de proyección \[P^{s}_{a_j,b_k}=|a_j b_k\rala a_j b_k|+|b_k a_j\rala b_k a_j|,\] además los estados $|a_j b_k\rala a_j b_k|$ son ortogonales con $|b_k a_j\rala b_k a_j|$. El operador $P^s_{a_j,b_k}$ se interpreta como la falta de conocimiento si el estado se encuentra en $|a_j b_k\ra$ o en $|b_k a_j\ra$. 

Y sea $R$ el conjunto de pares ordenados $(a_i,b_l)$ de valores propios que tienen un operador de proyección respectivo tal que $|a_i b_l\rala a_i b_l|=|b_l a_i \rala b_l a_i |$, \[P^r_{a_i,b_l}=|a_i b_l\rala a_i b_l|=|b_l a_i \rala b_l a_i |.\] Luego, con una probabilidad $q$ el instrumento cuántico es \[\sum_{j,k} P_{a_j,b_k}\otimes P_{a_j,b_k}\rho P_{a_j,b_k}\] y con una probabilidad $(1-q)$ es \[\sum_{(a_i,b_l)\in R} P_{a_i, b_l}\otimes  P^{r}_{a_i,b_l}\rho P^r_{a_i,n_l}+\sum_{(a_j,b_k)\in S}P_{a_j b_k} \otimes  \dfrac{1}{2}P^{s}_{a_j,b_k}\rho P^s_{a_j,b_k}.\]
...
\begin{equation}
    \begin{split}
        \mathcal{I}_3(\rho)&=q\sum_{a_j,b_k}  P_{a_j,b_k}\otimes P_{a_j,b_k}\rho P_{a_j,b_k} +(1-q)\sum_{(a_i,b_l)\in R} P_{a_i, b_l}\otimes  P^{r}_{a_i,b_l}\rho P^r_{a_i,b_l}\\
        &+(1-q)\sum_{(a_j,b_k)\in S}P_{a_j,b_k} \otimes  \dfrac{1}{2}P^{s}_{a_j,b_k}\rho P^s_{a_j,b_k}.\\
    \end{split}
\end{equation}  
...
\begin{equation}\label{tercera_alternativa_ejemplo1}
    \begin{split}
        \mathcal{I}_3(\rho)&=\sum_{a_j,b_k} q [P_{a_j,b_k}\otimes P_{a_j,b_k}\rho P_{a_j,b_k}]+(1-q)[P_{a_0,b_0}\otimes P_{a_0,b_0}\rho P_{a_0,b_0}+P_{a_1,b_1}\otimes P_{a_1,b_1} \rho P_{a_1,b_1}\\
