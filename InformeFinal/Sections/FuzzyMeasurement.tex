...
\end{equation}
$S_{ij}$ es la operación de intercambio \textit{SWAP} con respecto a las
partículas $i$ y $j$ del estado del sistema {\cite{Pineda_2021}} \cpnote{esta frase esta 
muy larga. partela por favor}. Esto puede
...
realiza la medición del observable $B\otimes A$ (notar que a veces $[A\otimes
B,B\otimes A]\ne 0$) \cpnote{Se me hace que esa aclaración no va a lugar. para que 
la pones?}. 
...
\end{equation}
donde el sistema clásico servirá como un apuntador que indica que operadores
aplicar al estado inicial\cpnote{no me gusta como está. PAreciera que el sistema clasico funciona
para decidir que operacion aplicar y realmente no es asi. como que la funcion principal es 
la de dar un resultado definido. lo platicamos?}. Esto es muy útil debido a que es una forma resumida
de describir completamente la medición difusa y no es necesario considerar
mediciones selectivas\cpnote{Tampoco entiendo esta frase acá. Pareciera una justificación, 
pero entonces no va acá sino como en la ``introduccion'' de la subsección.}.


Para obtener la salida de la medición y su respectiva probabilidad se puede
realizar la traza parcial sobre el sistema cuántico y para obtener el mapeo
completamente positivo y que preserva la traza para una medida no selectiva
basta con trazar parcialmente sobre el sistema clásico o también llamado
apuntador.\cpnote{Acá me gustaria que pusieras las ecuaciones de manera explicita.}

Utilizando instrumentos cuánticos las mediciones difusas se pueden entender
como tres alternativas diferentes. 
...
guardar estos ejemplos que siguen para tu tesis.  }
...
acerca de los operadores $E_\alpha$. Es por ello que para describir el efecto
...
será un nuevo operador de Kraus que cumpla con definir la misma medida POVM\@.
