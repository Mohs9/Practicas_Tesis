...
también. \cpnote{Revisa la fisica de este enunciado. Creo que no l oentiendes. También 
porfa simplifica la frase, partiendola en do spartes y ajustando. Leyendola por segunda vez quizá
solo requiere un pequeño ajuste. A ver si lo ves. }
    \end{itemize}
\end{definition}

En el caso de las mediciones resulta conveniente el primer axioma, se hace la
convención de que $\E$ no necesariamente preserve la traza de los operadores de
densidad, $\tr(\rho)=1$ \cpnote{Esta muy criptico}. Para verlo mejor, suponga que se realiza una medición
...
para que decir esto si ya se dijo lo de las probabilidades?}
...
$\E_\alpha$ debe ser lineal y convexo\cpnote{De nueuvo, porque convexo? 
porque no basta con lineal?} \cite{preskill2020quantum}
\begin{equation}
...
positivos a operadores positivos {\cite{nielsen_chuang_2010}},
\cite{preskill2020quantum}}. 
\cpnote{La forma de citar mas de una fuente es asi:
\cite{nielsen_chuang_2010,preskill2020quantum}, por favor corrige :) }. 
...
\cpnote{Este parrafo está mal, me paree. De donde lo sacaste? No entendi la linea de razonamiento 
para decir que la traza parcial no es valida.}
...
la \textit{representación como suma de operadores}  \cpnote{Mejora la redaccion}. La representación de suma
...
la condición $\sum_k K_i^\dagger K_i\le \mathds{1}$ \cpnote{corrige la ecuacion}.
