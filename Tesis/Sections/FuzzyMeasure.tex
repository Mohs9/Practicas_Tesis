...
En una medición ideal en la que se desea medir el observable de la forma $A\tensor B$ para un estado inicial $\rho$, los operadores que describen completamente esta medición son los operadores de proyección del observable, en este caso $P_{a_j, b_k}$ debido a que estos proporcionan la probabilidad de obtener cada una de las salidas posibles. La probabilidad de obtener la salida $a_j b_k$ será $\tr(P_{a_j,b_k}\rho )$ y el estado posterior a la medición estará dado por $\dfrac{P_{a_j,b_k}\rho P_{a_j,b_k}}{\tr(P_{a_j,b_k}\rho )}$.Además el valor esperado de esta medición es simplemente \[\sum_{j,k}a_j b_k\tr(P_{a_j,b_k}\rho)= \tr(A\otimes B \rho)=\langle A\otimes B\rangle_{\rho},\] sin embargo este valor esperado no es el mismo que el presentado en la ecuación {\eqref{eq:Expected-Value-FM-2p}}. Es por ello que para describir una medición difusa estos operadores no son suficientes por si solos, puesto que la probabilidad de obtener cada una de las salidas ahora está condicionada a la posibilidad $p$ de identificar de manera equivocada a las partículas. En está medición la probabilidad de cada una de las salidas está capturada por la acción del operador difuso en el estado inicial, en general $\tr(P_{a_j,b_k}\fuzzy{\rho})\ne\tr(P_{a_j,b_k}\rho ) $, y a su vez el estado posterior no puede estar determinada por la simple proyección a los espacios propios del observable que se desea medir. 


%\cpnote{Cual es la utilidad? porque aca?} \rrnote{Más bien es revisitar la utilidad pero para las mediciones difusas particularmente, son útiles porque con ellas es posible realizar el mapeo de probabilidades de forma selectiva.}\cpnote{Nop entiendo. Croe que quizá nos toca hablar de este punto. Preparate y me dices cuando hablamos} 

\rrnote{Esqueleto:En este parrafo, describir de forma corta la utilidad de las medidas POVM\@. Explicar que la medidas POVM son útiles porque con ellas es posible realizar un mapeo qde probabilidades para describir la medición difusa. Este mapeo que toma dos argumentos. El primero es una salida de medición $\alpha \in S $, donde $S$ es el conjunto de todos los posibles resultados de la medición, y el segundo es un estado cuántico. }  \rrnote{volvi a reescribir la idea del esqueleto}





\rrnote{Esqueleto: Explicar la relación de los operadores de Kraus y con las medidas POVM\@. Para describir una medida difusa también se necesita el estado posterior a la medición y este no puede ser especificado solamente por una medida POVM\@. Es por ello que para describir el efecto de la medición difusa en un estado de entrada $\rho$, es conveniente establecer un conjunto de operadores de Kraus $\{K_\alpha\}$ tales que $E_\alpha=K_\alpha^\dagger K_\alpha$ } \rrnote{volvi a reescribir la idea del esqueleto}
...
\rrnote{Esqueleto: Finalmente, describir como se usará el operador difuso y el
valor esperado para obtener las medidas POVM, en un sistema de dos partículas para simplificar la explicación.}.

Es posible utilizar el valor esperado calculado en la ecuación {\eqref{eq:Expected-Value-FM-2p-desarrollo}} y reescribirlo de la siguiente forma \begin{equation}\label{eq:ordenar-valoresperado-povm}\begin{split}
...
\end{split}
\end{equation}donde con $a_j$ y $b_k$ son los valores propios de los operadores $A$ y $B$ respectivamente y $P_{a_j,b_k}=|a_j b_k\rala a_j b_k|$ son lo operadores de proyección del observable $A\otimes B$ y $P_{a_j,b_k}$ son los operadores de proyección de $B\otimes A$. Los operadores de Kraus se obtienen al realizar alguna descomposición apropiada de estos operadores.
