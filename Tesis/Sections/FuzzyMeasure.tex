...
%\cpnote{Cual es la utilidad? porque aca?
 \rrnote{Más bien es revisitar la utilidad pero para las mediciones difusas
particularmente, son útiles porque con ellas es posible realizar el mapeo de
probabilidades de forma selectiva.}\cpnote{Nop entiendo. Croe que quizá nos
toca hablar de este punto. Preparate y me dices cuando hablamos} 
...
completamente positivos que preservan la traza. La acción de un instrumento
cuántico sobre un operador de densidad $\rho$, es el siguiente canal cuántico,
que presenta una salida clásica y cuántica: \cpnote{Esta rara la frase ``La accion es
el siguiente canal cuantico'', arregla}
\begin{equation}
    \begin{split}
...
mapeo de resultados realizando la traza parcial sobre el sistema cuántico. De
...
\end{split}
\end{equation}
\cpnote{que es la qu en la traza? Creo qeu no lo has definido. lo mismo para la otra ecuacion. }
Por otro lado, al trazar parcialmente sobre el sistema clásico se obtendrán
las operaciones completamente positivas que preservan la traza para mediciones
no selectivas
\begin{equation}
    \begin{split}
...
resultados, vale la pena considerar una medida de central que caracterice el
