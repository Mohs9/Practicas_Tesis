...
\rrnote{Los operadores de proyección no son suficiente para describir mediciones difusas porque el sistema experimenta transformaciones aleatorias, como el posible intercambio de partículas. Al realizar la medición de un observable, la probabilidad de obtener cada una de las salidas ahora está condicionada a la posibilidad de identificar de manera equivocada a las partículas, y el estado posterior también difiere de una medición ideal.}

\rrnote{Esqueleto:En este parrafo, describir de forma corta la utilidad de las medidas POVM\@.}
\cpnote{Cual es la utilidad? porque aca?} \rrnote{Más bien es revisitar la utilidad pero para las mediciones difusas particularmente, son útiles porque con ellas es posible realizar el mapeo de probabilidades de forma selectiva.}

\rrnote{Esqueleto: Explicar la relevancia de los operadores de Kraus y su
relación con las medidas POVM\@. }
\cpnote{Porque aca? Que son?}  \rrnote{ Porque quiero vincular los operadores de POVM y los operadores de Kraus para proponer el estado posterior a la medición difusa}
