...
\cpnote{No me gusta el titulo de esta subseccion. Discutamoslo}\rrnote{Me parece bien}
...
En esta sección se ahonda en la relevancia del valor esperado y su aplicación en este estudio. En el apartado {\ref{sec:OpDensidad}} se aludió al valor esperado al referirse por primera vez al valor promedio de un observable $\mathcal{O}$, cuando se realiza un gran número mediciones, sin embargo no se profundizó en el tema. A continuación, se abordará el concepto de valor esperado y se detallará específicamente qué ocurre con esta medida en mediciones difusas.  


\cpnote{En la intro arranca con lo más importante y luego expandes. Tambien
podemos discutir un poco a que me refiero} \rrnote{Está bien, lo cambié, pero me gustaria hablarlo también }
...
%observables cuando el sistema se encuentra en un estado arbitrario
\cpnote{claro que si}. %Se suele afirmar que, en términos generales, el valor de los observables no está determinado
\cpnote{error}. %Por esta razón y con el propósito de relacionar las predicciones con los resultados experimentales, se introduce el concepto de valor esperado{\cite{gomez2010introduccion}}
\cpnote{No me gusta. Hay problemas}.\rrnote{Está bien, esto lo tomé del libro pero lo quitaré porque no aportaba realmente.}
...
El valor esperado es un término que se deriva de la teoría básica de la probabilidad y representa el promedio ponderado de los resultados posibles que se pueden obtener, cuando se dispone de muchos sistemas, todos en el mismo estado inicial, y se realiza la misma medición en cada uno de ellos. La ponderación que se le asigna a cada resultado se determina por la probabilidad de obtener dicho resultado {\cite{gomez2010introduccion,sakurai2017modern}}.\cpnote{Ojo, el lenguaje aca cambio. Esto lo copiaste casi literal de alguna fuente? Si es asi, 
te toca borrarlo y escribirlo con tus propias palabras. Lo mismo para cualquier parte. Una frase completa casi literal igual es problematica.}\rrnote{No todo lo copie literal, pero si la habia tomado la última frase casi literal entonces ya lo corregí}
...
luego ya escribes los parrafos}\rrnote{Reduje los dos párrafos a uno y al inicio coloque las dos ideas principales del parrafo.}
...
%sistema que experimenta cambios aleatorios
\cpnote{como que no? }\rrnote{quité esta parte que no me habia expresado bien.}
...
parrafo? Porque hablar de operaciones cuánticas? }\rrnote{inteoduje esto porque es lo que entiendo de la forma general de los valores esperados}
...
\rrnote{Esqueleto: Los POVM y los operadores de Kraus se hablaron en el marco teórico y se verán en las implementciones. En esta parte se mencionará como utilizar los operadores POVM y los operadores de Kraus conjuntamente para describir mediciones imperfectas y específicamente en mediciones difusas.}

\rrnote{Esqueleto: En este parrafo se menciona que en una medicion ideal los operadores que describen completamente  una medicion  son los operadores de proyeccion, sin embargo en operaciones difusas no son suficientes. Dar la razon por que los operadores de proyeccion son insuficientes para describir completamente una medición difusa.}

\rrnote{Esqueleto:En este parrafo,  revisitar las medidas POVM de forma corta y escribir su utilidad.}

\rrnote{Esqueleto: Explicar la relevancia de los operadores de Kraus y su relación con las medidas POVM\@. }

\rrnote{Esqueleto: Finalmente en el útlimo párrafo, describir como se implementarán en mediciones difusas haciendo uso del operador difuso.}
