...
difusas.\cpnote{Normalmente reservamos la $\mathcal{H}$ para espacios de Hilbert. Checa
a ver que otra notación usa la gente} \rrnote{ Lo cambié por como lo vi en algunos libros y en internet. }
...
\rrnote{Esqueleto Intro: En esta sección se introduce el concepto de
instrumento cuántico. Se explica la función de ellos además de detallar en cómo
se calcula el valor esperado del instrumento y la razón de emplearlos. Además
se especifican las dos alternativas que se usan.}
\cpnote{Dices que se especifican las dos alternativas que vas a usar, pero finalmente
no lo pones.}\rrnote{ Sí, lo siento esa parte la borré por error, pero ya la agregué y en realidad son tres alternativas}




Un \textit{instrumento cuántico} es una representación matemática que integra  los conceptos de medición y operación cuántica, registrando tanto la información clásica como la cuántica. Esta herramienta es útil para aproximarse a describir completamente las mediciones de forma resumida y sin necesidad de considerar mediciones selectivas, siendo especialmente útil para trabajar con mediciones difusas. En esta sección se detalla algunas de sus características más importantes en la especificación de las mediciones difusas, además del cálculo para obtener su valor esperado.  Adicionalmente, se presentan tres alternativas de instrumentos cuánticos que representan distintas interpretaciones para la especificación entera de mediciones difusas.

\cpnote{Creo qeu se da la impresion que los instrumentos cuánticos son utiles
solo para mediciones difusas, pero no es asi. Quizá convenga reformular esta
frase (y otra mas adelante de este párrafo)}\rrnote{Reformulé el párrafo anterior}



\rrnote{Esqueleto: En este párrafo simplemente se explica qué es un instrumento
cuántico. Un instrumento cuántico se compone  de una base ortonormal
$\{|\alpha\rala \alpha |\}$ e incluye al conjunto de mapeos
$\{\E_\alpha(\rho)\}$. La acción de un instrumento cuántico sobre un operador
de densidad $\rho$ es un canal cuántico, que presenta una salida clásica y
cuántica. }

Un instrumento cuántico se define de la siguiente manera {\cite{wilde2011classical}}.\begin{definition}[\textbf{Instrumento Cuántico}] Es un mapeo que toma un operador de entrada y tiene como salida un ensamble que correlaciona un sistema clásico, que contiene la salida de la medición y un sistema cuántico, que contiene el estado posterior a la medición.  Sea $\{|\alpha\rangle \}$ la base ortonormal para un espacio de Hilbert $\mathcal{H}_{\alpha}$. Además, sea $\{\E_\alpha(\rho)\}$ el conjunto de mapeos completamente positivos que preservan la traza. La acción de un instrumento cuántico sobre un operador de densidad $\rho$, es el siguiente canal cuántico, que presenta una salida clásica y cuántica:\begin{equation}
    \begin{split}
...
\rrnote{Esqueleto: Exponer cuál es la utilidad de los instrumentos, y como obtener los elementos que describen completamente una medición. Esta herramienta establece una conexión directa entre los estados y los resultados de la medición. Por consiguiente, ya no es preciso tener en cuenta mediciones selectivas de manera explícita. Además, es posible obtener el mapeo de los resultados mediante la traza parcial del sistema cuántico.}


Esta nueva herramienta vincula los estados directamente a los resultados de la medición. Por esta razón, ya no es necesario considerar mediciones selectivas, explícitamente.  Es importante destacar que, en principio, se puede obtener el mapeo de resultados realizando la traza parcial sobre el sistema cuántico. De esta manera, se obtiene el operador que modela la salida de la medición y su respectiva probabilidad \begin{equation}
    \begin{split}
...
\rrnote{Esqueleto: Con base en lo anterior, me gustaria decir por qué es util hablar del valor esperado de los instrumentos cuánticos. Dado que con los instrumentos cuánticos se puede obtener un mapeo de resultados, vale la pena considerar una medida de central que caracterice el mapeo que se obtiene con ellos.} 
...
\rrnote{Esqueleto: Detallar cómo se realiza el cálculo del valor esperado de un instrumento cuántico y cuál es la idea detrás de éste cálculo. El valor esperado para un instrumento cuántico se calcula como $\la\mathcal{O}\ra_{\mathcal{I}(\rho)}=\tr\left[\mathcal{I}(\rho)(\mathcal{O}\tensor\mathds{1})\right]$. El valor esperado es el promedio ponderado de las salidas. En consecuencia, al desarrollar esta expresión se puede ver que  es un promedio ponderado de las posibles salidas de la medición, donde la probabilidad es el peso asignado a cada una de las salidas}
