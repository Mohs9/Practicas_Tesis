...
En el apartado {\ref{sec:OpDensidad}} se aludió al valor esperado al referirse por primera vez al valor promedio de un observable $\mathcal{O}$, cuando se realiza un gran número mediciones. En esta sección se ahonda en la relevancia de este promedio y su aplicación en este estudio.


\rrnote{Explicar qué es y cómo se interpreta el valor esperado en mecánica cuántica.}


En los sistemas cuánticos, no es posible asignar un valor definido a los observables cuando el sistema se encuentra en un estado arbitrario. Se suele afirmar que, en términos generales, el valor de los observables no está determinado. Por esta razón y con el propósito de relacionar las predicciones con los resultados experimentales, se introduce el concepto de valor esperado {\cite{gomez2010introduccion}}.  

El valor esperado es un término que se deriva de la teoría básica de la probabilidad y se refiere al promedio ponderado de los valores propios que se podrían obtener en la medición. La ponderación de cada valor propio en el promedio es igual a la probabilidad de obtener dicho valor propio {\cite{gomez2010introduccion,sakurai2017modern}}. 

Es de suma importancia distinguir entre el valor esperado y los posibles valores que se pueden obtener en una medición del observable. Los valores que se pueden obtener deben ser los valores propios del observable
mientras que el valor esperado no necesariamente coincidirá con estos {\cite{gomez2010introduccion}}. Esto significa que si se tiene muchos sistemas, todos en el mismo estado, y se realiza la misma medición en cada uno, los resultados diferirán pero el promedio será lo presentado en la ecuación {\eqref{expectationvalue_traza}}. 
...
El cálculo del valor esperado como la traza del producto del observable $\mathcal{O}$ por el estado inicial del sistema, ya no es aplicable para un sistema que experimenta cambios aleatorios. Estos cambios en el estado inicial, que ocurren como resultado de algún proceso físico pueden ser capturados por las operaciones cuánticas {\cite{nielsen_chuang_2010}}. Si $\rho$ es el estado inicial antes del proceso, y $\mathcal{\rho}$ entonces el valor esperado de este estado final se puede expresar como \begin{equation}\label{eq:Expected-Value-Op-Cuanticas}
    \la \mathcal{O}\ra_{\mathcal{E}(\rho)}=\tr(\mathcal{E}(\rho)\mathcal{O})
\end{equation} 

Considerando lo anterior, se puede analizar una medición difusa en un sistema de dos partículas, en la que el operador difuso $\mathcal{F}_{2\text{p}}$ registra la evolución del estado inicial $\rho$. El valor esperado del observable factorizable $A\otimes B$ estará dado por {\cite{Pineda_2021}}\begin{equation}\label{eq:Expected-Value-FM-2p}
    \begin{split}
