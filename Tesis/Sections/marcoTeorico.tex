...
Con el propósito de lograr los objetivos de esta tesis  \cpnote{redundante} \rrnote{Cambie la primera frase}
...
otro lado, las medidas POVM, también llamadas medidas generalizadas, son apropiadas debido a que consideran la interacción del sistema con su entorno.\cpnote{mala redaccion} \rrnote{Borré unas frase y separe ideas.}Estas medidas son especialmente útiles
porque proporcionan las estadísticas de medición. Por último, el formalismo de las operaciones cuánticas describe los efectos de la medición en un sistema cuántico y representa la descripción de los cambios  de estados discretos, esto es transformaciones entre el estado inicial $\rho$ y el estado final $\rho'$, sin necesidad de referenciar al paso del tiempo {\cite{nielsen_chuang_2010}}\cpnote{no solo
eso, hablar con Rubi}.  


En la sección {\ref{sec:Cap1:OpDensidad}}\cpnote{Me confunde un poco
esto. Esta es la primera seccion. Ayudar a Rubi con la 
numeracion y/o corregir que esta no tenga seccion. Decir que es 
mejor con las refs y labels}\rrnote{ Ya lo compuse con labels y ref }, se presentan la definición y las propiedades del
...
{\cite{sakurai2017modern}} \begin{equation}
 	...
 	\end{split}
\end{equation} 
donde $\{|\phi_j\rangle \}$ es una base ortonormal \cpnote{Que es una base mas general? 
en tu caso, la base es ortogonal? Normalizada?}\rrnote{Sí, me refería a ina base ortonormal }
...
({\ref{defdensityoperator}})\cpnote{No veo porque...}\rrnote{cambie los parentesis que causaban confusión}. Notar que $\langle \phi_k| \rho|\phi_j\rangle$ son los elementos de la matriz $\rho$, escrita en la base $|\phi_j\rangle$. Por lo tanto, el valor promedio del observable es {\cite{sakurai2017modern}}\begin{equation}
	...
	\end{split}
\end{equation}
\cpnote{croe qeu sobran cosas aca para hacerlo mas claro. De la ecuacion anterior, los parentesis no adyudan tampoco}\rrnote{Quite algunas frases y agregué otras.}
...
\end{equation}
\cpnote{La ecuacion está mal}\rrnote{agregué la suma.}
...
\cpnote{Los postulados ya lo estas diciendo acá. Me parece un poco repetitivo 
que los des de nuevo en la siguiente seccion. Hablamos a ver que hacemos.} \rrnote{Es verdad, a mi megustaria quitar esta parte y dejar los postulados en la siguiente.}
...
estado $\rho_i$, entonces \cpnote{redaccion} \rrnote{cambié lo anteiror}el estado del sistema total será
...
\cpnote{Esto lo vas a ocupar en la tesis?}\rrnote{ Esto directamente no, solo sirve para entender el toerema en la seccion 1.4 que tampoco he usado pero no sé si lo usaré en algun punto (no creo.)}
...
sistema cuántico y su discusión se llevará a cabo a continuación. \cpnote{redundante}\rrnote{quité lo último.}
...
estado posterior a la medición del sistema.\cpnote{repetitivas las ultimas frases}\rrnote{quité esas frases.}
...
\cpnote{Esto ya lo habías dicho. Porfa dale una pasada y trata de no repetir conceptos}\rrnote{quité donde hablaba de esto antes, porque ne parecio que antes estaba la idea desconectada y solo lo dejé acá}
...
\end{equation} donde los operadores de medición del conjunto ortogonal $\{P_m\}$ satisfacen la relación de completitud {\cite{2007geometry}}. \cpnote{redaccion}\rrnote{cambié la redacción.}
...
\end{equation} 
%El estado conmuta con el observable $\mathcal{O}$. El valor esperado del observable estará dado por 
%\begin{equation}
%	\la \mathcal{O} \ra=\sum_m p_m\lambda_m=\sum_m \lambda_m
%\tr(P_m\rho)=\tr(\mathcal{O}\rho).
%\end{equation}
\cpnote{Para que este parrafo? Además esta raro de redaccion} %Una característica clave de las mediciones proyectivas consiste en la repetibilidad. Esto significa que si la medición es repetida, el estado en ({\ref{stateProjective}}) sigue siendo el mismo, y da el mismo resultado {\cite{2007geometry}}.
\cpnote{Para que queremos decir esto?}\rrnote{ Este párrafo estaba de más. }
% }}}
\subsection{Operadores POVM}\label{subsec:operadoresPOVM} % {{{
\cpnote{Lo de las mediciones ya lo has dicho varias veces. Vas a tener que darle una organizada a la intro para no repetir ideas y que quede un poco mejor.}\rrnote{recorté la introducción para no repetir}
...
Los sistemas reales sufren interacciones indeseadas con el mundo exterior, éstas aparecen como ruido en información cuántica. Las operaciones cuánticas son un herramienta capaz de describir dicho ruido cuántico y el comportamiento de sistemas cuánticos abiertos.  %Adicionalmente, .
