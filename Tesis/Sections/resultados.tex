...
medir un observable ${A \tensor B}$ en el sistema representado por el estado $\rho_{1,2}$, se obtiene un resultado  en
...
En la imagen {\ref{diagrama-cajas-primer-instrumento}} \cpnote{A veces latex mueve las imagenes. Mejor refierete a 
ella por medio del comando ref y en la siguiente frase ya hablas de ``la figura'' para
no repetir la referencia.}\rrnote{de acuerdo, agregé lareferencia} se puede apreciar a lo que esto se refiere gráficamente.
En la figura {\ref{fig:1-instrumento-ideal}} con una probabilidad $p$ se
realiza una medición ideal, se desea medir el observable $A$ en el subsistema
de la primera partícula y el observable $B$ en el subsistema de la segunda partícula. El estado posterior a la
medición será la proyección del estado $\rho_1=\tr_2(\rho_{1,2})$ que representa al sistema correspondiente a la primera partícula \cpnote{rho 1 no es un sistema. 
...
conjuntamente con la proyección del estado $\rho_2=\tr_1(\rho_{1,2})$ al espacio propio de las
...
del observable $A$ se realiza en el segundo sistema, en consecuencia el
estado $\rho_2$ se proyecta al espacio propio que corresponde a la salida
...
\end{split}\]Es importante notar que este mapeo de resultados resultante es exactamente el mismo al de la ecuación {\eqref{eq:mapeo-probabilidades-2p}}
\cpnote{esta frase aparece suelta. Conecta mejor ocn la ecuacion}.\rrnote{Conecte la frase anterior} 

Por otro lado, para obtener el estado posterior luego de haber realizado la medición, se debe realizar la traza parcial sobre el sistema clásico\cpnote{También esta frase está como volando}. En este caso es fácil ver que
