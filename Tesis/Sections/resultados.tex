...
El primer instrumento consiste en considerar que debido a un error en el sistema cuántico, las partículas posiblemente experimenten un intercambio. Al medir un observable ${A \tensor B}$ en el sistema, se obtiene un resultado  en el sistema clásico que puede ser cualquiera de los valores propios de este observable. Luego, con cierta probabilidad el estado posterior será la proyección del estado inicial del sistema al estado propio correspondiente a las salida proporcionada. Sin embargo también es posible que el estado inicial sufra una transformación, y sea este cambio del sistema el que se proyecte en el espacio propio correspondiente a la salida del observable $A\tensor B$.


En la siguiente imagen se puede apreciar a lo que esto se refiere gráficamente. En la figura {\ref{fig:1-instrumento-ideal}} con una probabilidad $p$ se realiza una medición ideal, se desea medir el observable $A$ en el sistema $\rho_1$ y el observable $B$ en el sistema $\rho_2$ y el estado posterior a la medición será la proyección del sistema $\rho_1$ al espacio propio correspondiente a la salida proporcionada en el sistema clásico $a_j$  conjuntamente con la proyección del sistema $\rho_2$ al espacio propio de las salida $b_k$. No obstante, la figura {\ref{fig:1-instrumento-non-ideal}} muestra la parte borrosa de la medición en la que con una probabilidad $1-p$, la medición del observable $B$ se realiza en el sistema $\rho_1$ y la medición del observable $A$ se realiza en el sistema $\rho_2$, en consecuencia el sistema $\rho_2$ se proyecta al espacio propio que corresponde a la salida $a_j$ y $\rho_1$ se proyecta al espacio de la salida $b_k$.
...
Es importante notar que el sistema clásico  puede diferir del estado inicial, pero  indica correctamente el estado posterior a la medición en todos los casos. Además, para una medida no selectiva el mapeo de resultados se obtiene simplemente aplicando una traza parcial sobre el sistema cuántico \begin{equation}\label{eq:traza_q_1instrument2p}
    \begin{split}
...
Para obtener el mapa de resultados para una medición selectiva simplemente se multiplica el operador de proyección correspondiente a la salida obtenida al resultado anterior, y se calcula la traza\[\begin{split}
...
\end{split}\] El cual es exactamente el mismo al de la ecuación {\eqref{eq:mapeo-probabilidades-2p}}. Mientras que el estado posterior se obtiene realizando la traza parcial sobre el sistema clásico. En este caso es fácil ver que \begin{equation}\label{eq:traza_cl_1instrument2p}
    \begin{split}
...
\end{split}
\end{equation} Para una medida selectiva el estado posterior estará dado por \[\E_{a_j,b_k}(\rho)=P_{a_j,b_k} \mathcal{F}_{2\text{p}}(\rho) P_{a_j,b_k}.\]
